...
def_sum <- df %>%
  ...
  summarise(def_r = mean(liwale_def_fac)*100,
            def_sd = sd(liwale_def_fac),
            def_se = def_sd / sqrt(length(liwale_def_fac)))
# Avg T vs F
def_sum2 <- def_sum %>%
  group_by(VLFR_fac) %>%
...
E_VLFR <- (1-(1.46/9.55))*100 # E for 2018-2022. Add to plot
E_ann <- (1-(0.292/1.91))*100
...
dot <- ggplot(def_sum2, aes(x = VLFR_fac, y = def, colour = VLFR_fac)) +
  ...
  theme(
    ...
    plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), 
                       "inches")) +
  labs(y= "Mean Deforested Area 2018-2022 (%)", x = "Management Type") +
  annotate("text", x = 1, y = -2.5, label = "*Data from Mbilinyi and Zahabu, 2024", size =5) +
  coord_cartesian(ylim = c(0, 12), clip = "off") +
  guides(colour = "none")

ggsave("/exports/csce/datastore/geos/users/s1318698/FORVAC_matching/VLFR_outside_dot.png", dot, width = 20, height = 20, units = "cm", dpi = 300)
...
inc <- ggplot() +
  ...
  ylab("Mean Deforestation Rate 2018-2022 (%)") +
  annotate("text", x = 4.5, y = 16.5, label = "E", size = 6) +
  annotate("text", x = 4, y = 16.5, label = "97 %", size = 6) +
  annotate("text", x = 3, y = 16.5, label = "90 %", size = 6) +
  annotate("text", x = 2, y = 16.5, label = "78 %", size = 6) +
  annotate("text", x = 1, y = 16.5, label = "51 %", size = 6)
